...
static int bootm_load_os(bootm_headers_t *images, int boot_progress)
{
	...
	//debug("VVVV: before no_overlap:%x, load:%lx, blob_end:%lx, load_end:%lx, blob_start:%lx\n", no_overlap, load, blob_end, load_end, blob_start);
	if (images->fit_rfs_size && images->fit_rfs_offset && (blob_end > images->fit_rfs_size))
		blob_end -= images->fit_rfs_size;
	//debug("VVVV: after no_overlap:%x, load:%lx, blob_end:%lx, load_end:%lx, blob_start:%lx\n", no_overlap, load, blob_end, load_end, blob_start);
#endif
