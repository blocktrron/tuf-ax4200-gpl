		   ...
		   enum fit_load_op load_op, ulong *datap, ulong *lenp)
{
	...
	if (image_type == IH_TYPE_RAMDISK && !strcmp("rootfs-1", fit_uname)) {
		size_t rd_size;
		const void *rd_data;

		if (fit_image_get_data_and_size(fit, noffset, &rd_data, &rd_size) == 0) {
			//printf("fit:%p, rd_data:%p, szie:%zx\n", fit, rd_data, rd_size);
			images->fit_rfs_offset = rd_data - fit;
			images->fit_rfs_size = rd_size;
			//printf("images rfs_offset:%lx, rfs_size:%lx\n", images->fit_rfs_offset, images->fit_rfs_size);
			return -ENOPKG; // tell bootm don't load this image
		}
	}
#endif
