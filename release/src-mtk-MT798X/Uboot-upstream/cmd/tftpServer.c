...
int program_bootloader(ulong addr, ulong len)
{
	void *hdr = (void *)addr;
	int ret = 0;
	const ulong max_xfer_size = CONFIG_MAX_TRX_IMAGE_SIZE;
...
#if defined(TUFAX6000) // SPI nand flash stability workaround
int gl_crc32_err_cnt;
#endif
...
#endif
	if (ret) {
#if defined(PLAX56_XP4) || defined(ETJ) || defined(RTAX59U)
		asus_purple_led_on();
#endif
#if defined(TUFAX6000) // SPI nand flash stability workaround
		if (gl_crc32_err_cnt >= 2) {
			printf("!!!!AX6K workaround, reset it!!!!\n");
...
int do_gomenu(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
	jump_to_menu_select();
	return 0;
}
...
U_BOOT_CMD(
	gomenu, 1, 1, do_gomenu,
	"gomenu\t - goto main menu\n",
	NULL
);
...
#if defined(PLAX56_XP4) || defined(ETJ) || defined(RTAX59U) /* rescue process indication */
...
static void RAMtoFlash(void)
{
	...
	if (!memcmp(ptr, trx_magic, sizeof(trx_magic)) && (copysize > CONFIG_MAX_TRX_IMAGE_SIZE))
	{
	...
	else
	{
		...
		if (CONFIG_MAX_TRX_IMAGE_SIZE > 0x30000)
			magic_offset = CONFIG_MAX_TRX_IMAGE_SIZE - 7;

		printf("Magic number offset: %x\n", magic_offset);
		q = (char*)(ptr + magic_offset);
		printf("upgrade boot code\n");	// tmp test

		if (copysize > 0 && copysize <= CONFIG_MAX_TRX_IMAGE_SIZE)		// uboot
		{
		...
		else if (copysize > CONFIG_MAX_TRX_IMAGE_SIZE)
		{
			parseflag = 0;
			printf("    Download of 0x%x bytes completed\n", copysize);
		}
		...
		if (parseflag != 0)
			copysize = CONFIG_MAX_TRX_IMAGE_SIZE;
