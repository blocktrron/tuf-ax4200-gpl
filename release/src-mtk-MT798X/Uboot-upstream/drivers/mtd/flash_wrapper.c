...
extern struct mtd_info *gl_spinand_info[];
extern int spi_curr_device;
#else
#error UNKNOWN NAND !!
#endif
...
#elif defined(CONFIG_MTD_SPI_NAND)
	if (spi_curr_device == -1)
		return NULL;
	return gl_spinand_info[spi_curr_device];
#else
#error UNKNOWN NAND !!
#endif
}
...
int init_ubi_volumes(struct ubi_device *ubi)
{
	...
	const struct ubi_vol_s *linux_p = NULL;
	struct ubi_volume *linux_v = NULL;
#endif
...
#if defined(TUFAX4200)
			// old flash layout translation workaround
			if (!strcmp(p->name, "linux")) {
				linux_p = p;
				linux_v = v;
			}
#endif
...
#if defined(CFG_DUAL_TRX)
		if (mtd->size < 0x10000000 && !strcmp(p->name, "linux2")) // smaller than 256MB, old 128MB SR sample
			continue; // don't create linux2
#endif
		show++;
	...
	// old flash layout translation workaround
	if (linux_v && linux_p) {
		if (linux_v->reserved_pebs == CFG_UBI_OLD_FIRMWARE_LEB) {
			char *ubi_remove_vol[] = { "ubi", "removevol", NULL};
			char *ubi_read_vol[] = { "ubi", "readvol", tmp, NULL, tmp1};
			char *ubi_write_vol[] = { "ubi", "writevol", tmp, NULL, tmp1 };
			printf("!!!! translate old layout !!!!\n");
			// step1. back up old linux content
			size = linux_v->reserved_pebs * linux_v->usable_leb_size;
			sprintf(tmp, "%x", CONFIG_SYS_LOAD_ADDR);
			ubi_read_vol[3] = "linux";
			sprintf(tmp1, "%x", size);
			do_ubi(NULL, 0, ARRAY_SIZE(ubi_read_vol), ubi_read_vol);
			// step2. remove jffs2, linux2, linux
			ubi_remove_vol[2] = "jffs2";
			do_ubi(NULL, 0, ARRAY_SIZE(ubi_remove_vol), ubi_remove_vol);
			ubi_remove_vol[2] = "linux2";
			do_ubi(NULL, 0, ARRAY_SIZE(ubi_remove_vol), ubi_remove_vol);
			ubi_remove_vol[2] = "linux";
			do_ubi(NULL, 0, ARRAY_SIZE(ubi_remove_vol), ubi_remove_vol);
			// step3. create linux
			ubi_create_vol[2] = "linux";
			sprintf(tmp, "%x", linux_p->size);
			if (linux_p->type)
				strcpy(type, "dynamic");
			else
				strcpy(type, "static");
			do_ubi(NULL, 0, ARRAY_SIZE(ubi_create_vol), ubi_create_vol);
			// step4. copy backup to linux
			sprintf(tmp, "%x", CONFIG_SYS_LOAD_ADDR);
			ubi_write_vol[3] = "linux";
			sprintf(tmp1, "%x", size);
			do_ubi(NULL, 0, ARRAY_SIZE(ubi_write_vol), ubi_write_vol);
			// step5. reset, let original code to create linux2/jffs2 & copy linux to linux2
			udelay(300000);
 ...
 * @buf:	buffer address
 * @addr:	Absolute address to read (include CFG_SYS_FLASH_BASE).
 * @len:	length to read
 * @return:
 * 	0:	success
 *     -1:	invalid parameter
...
{
	int ret = 0;
	unsigned int flash_offset = addr - CFG_SYS_FLASH_BASE;
#if defined(CFG_UBI_SUPPORT)
#else
	char addr_buf[30], off_buf[30], len_buf[30];
	char *sf_read[] = { "sf", "read", addr_buf, off_buf, len_buf };
#endif

	if (!buf || !len || addr < CFG_SYS_FLASH_BASE) {
	...
	if (flash_offset >= CFG_UBI_DEV_OFFSET) {
		...
		char *vol;
		char addr_buf[30], len_buf[30], vol_offset_buf[30];
...
{
	int ret = 0;
	unsigned long flash_offset = addr - CFG_SYS_FLASH_BASE;
#if defined(CFG_UBI_SUPPORT)
#else
	char addr_buf[30], off_buf[30], len_buf[30];
 ...
 * 		If addr is less than CFG_SYS_FLASH_BASE, ignore it.
 * @len:	length to read
 * @return:
 * 	0:	success
 *  otherwise:	error
...
#endif
	if (!len || addr < CFG_SYS_FLASH_BASE) {
...
static int check_eeprom_set_checksum(unsigned char *buf)
{
	...
	if (hcrc != checksum) {
		debug("Header checksum mismatch. (%" PRIx32 "/%" PRIx32 ")\n", hcrc, checksum);
		return -3;
	}
...
#else	/* !CFG_UBI_SUPPORT */
	if (in_range(off, len, CFG_EEPROM_OFFSET, CFG_EEPROM_SIZE)
	   )
	{
...
int do_flash_erase_write(cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	...
	len = simple_strtoul(argv[3], NULL, 16);
	if (flash_addr < CFG_SYS_FLASH_BASE) {
		printf("flash_addr must greater than or equal to 0x%x\n", CFG_SYS_FLASH_BASE);
		return EINVAL;
	}
...
{
	const struct mtd_info *mtd = current_mtd();
...
{
	int r;
	unsigned char *mac = factory + CFG_MAC_OFFSET;
...
int asus_set_ethmac(void)
{
	...
	unsigned char default_mac[6] = {0x00, 0xaa, 0xbb, 0xcc, 0xdd, 0xe0};
	r = ra_factory_read(mac, CFG_EEPROM_OFFSET + CFG_MAC_OFFSET, 6);
