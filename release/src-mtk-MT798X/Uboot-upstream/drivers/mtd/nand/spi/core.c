...
#define MAX_SPI_NANDCNT	2
int spi_curr_device = -1;
struct mtd_info *gl_spinand_info[MAX_SPI_NANDCNT];
#endif
...
void clear_spinand(void)
{
	if (spi_curr_device != -1) {
		int i;
		for (i=0; i<=spi_curr_device; i++)
			del_mtd_device(gl_spinand_info[i]);
		spi_curr_device = -1;
		spi_nand_idx = 0;
	}
}
...
#if defined(CONFIG_ASUS_PRODUCT)
	if ((spi_curr_device+1) >= MAX_SPI_NANDCNT) {
		printf("!!!! too many SPI nand probed !!!!\n");
	} else {
		if (spi_curr_device == -1)
			spi_curr_device = 0;
		else
			spi_curr_device++;
		gl_spinand_info[spi_curr_device] = mtd;
	}
#endif
