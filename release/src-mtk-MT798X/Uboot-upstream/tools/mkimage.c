...
static void usage(const char *msg)
{
	...
	fprintf(stderr, "       %s -V ==> print version information and exit\n",
		params.cmdname);
#else
	fprintf(stderr, "       %s -V KERNEL_VER FS_VER SERIALNO EXTENDNO 0 0 0 0 0 0\n",
		params.cmdname);
#endif
	...
	while ((opt = getopt(argc, argv,
		...
		case 'V':
...
#else
			{
				int i;
				if ((argc-=10) < 0)
					usage("Invalid image ver info");
				memset ((unsigned char *)&params.tail_pre, 0x00, sizeof(TAIL));
				sscanf(argv[1], "%2" SCNu8 ".%2" SCNu8, &params.tail_pre.kernel.major, &params.tail_pre.kernel.minor);
				sscanf(argv[2], "%2" SCNu8 ".%2" SCNu8, &params.tail_pre.fs.major, &params.tail_pre.fs.minor);
				for(i=0; i<(MAX_VER*2); i++)
					sscanf(argv[i+3], "%2" SCNu8 ".%2" SCNu8, &params.tail_pre.hw[i].major, &params.tail_pre.hw[i].minor);
				argv+=10;
				params.vargv=1;
			}
			break;
